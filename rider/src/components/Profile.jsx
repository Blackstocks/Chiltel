	...
	useEffect(() => {
		if (profile) {
			setProfileData({
				...
				securityAmount: profile.securityDeposit.amount || 0,
	...
	const StatCard = ({ title, value, icon: Icon }) => (
		<div className="p-2 transition-all transform border border-blue-100 shadow-sm bg-gradient-to-br from-white to-blue-50 sm:p-3 rounded-xl hover:scale-105">
			<div className="flex items-center justify-between">
				<h3 className="text-xs font-medium text-blue-600 sm:text-sm line-clamp-1">
					{title}
				</h3>
				<Icon className="flex-shrink-0 w-4 h-4 ml-2 text-blue-500" />
			</div>
			<p className="mt-1 text-lg font-bold text-gray-800 sm:mt-2 sm:text-xl">
				{value}
			</p>
		</div>
	);
