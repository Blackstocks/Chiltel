...
const riderController = {
  ...
  async signup(req, res) {
    ...
    try {
      ...
      //generate request id for court case
      const options = {
        method: "POST",
        headers: {
  ...
  async getAssignedServices(req, res) {
    try {
      const services = await ServiceRequest.find({
        ...
        .populate("services.serviceId", "name estimatedDuration")
  ...
  async getActiveService(req, res) {
    try {
      const service = await ServiceRequest.findOne({
        ...
        .populate("services.serviceId", "name rateChart estimatedDuration")
        .select("-OTP");
  ...
  async acceptService(req, res) {
    try {
      ...
      if (rider.mode === "COMMISSION" && rider.balance <= 0) {
  ...
  async verifyCourtCase(req, res) {
    ...
    try {
      const response = await fetch(
        "https://api.invincibleocean.com/invincible/getCourtCaseV2/" +
          rider.requestId,
        options
      );
      const data = await response.json();
